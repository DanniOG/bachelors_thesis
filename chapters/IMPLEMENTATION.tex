...
DPS je čtyřvrstvá deska o výšce 1.6 mm s dvěmi signálovými vrstvy a dvěmi silovými vrstvami. Kde první(horní) vrstva je signálová a nachází se na ní veškeré elektronické komponenty. Druhá je společná zem, třetí je napájecí 3.3 V a poslední spodní vrstva je také signálová.
...
Celková cena DPS a potřebné materiály jsou:
\begin{table}[H]
    \label{tab:bom}
...
12 bit AD převodník je součástí MCU STM32F407ZGT6. AD převodník slouží k měření absolutní hodnoty tlaku z tlakových senzorů na obou větví pneumatického systému. Vzorkovací frekvence byla zvolena $f_s = 1 \ kHz$.
...
24 bitový AD převodník MCP3561 převádí hodnoty z diferenčního senzoru tlaku. Během měření MCP3561 je zapojeno podle schéma (\ref{fig:mcp3561_connection}) a vnitřní nastavení registrů je následovné $OSR = 256$, $GAIN = 1$, $PRESCALE = 1$, $GAIN = 1$, $BOOST = 1$.
